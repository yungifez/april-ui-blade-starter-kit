    ...
    <body class="min-h-full bg-background text-foreground antialiased">
        <header class="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-8">
            ...
            <nav class="flex items-center gap-2">
                @auth
                    <april:button-link href="{{ route('dashboard') }}">Dashboard</april:button-link>
                @else
        ...
        <main class="mx-auto max-w-7xl px-6 pb-20 pt-16 lg:px-8 lg:pt-24">
            <div class="max-w-3xl">
                ...
                <p class="mt-6 max-w-2xl text-lg leading-8 text-muted-foreground">A clean Laravel 13 foundation with Fortify authentication, native Blade forms, and April UI components that keep the Laravel way of working intact.</p>
                <div class="mt-8 flex flex-wrap gap-3">
                    @auth
                        <april:button-link href="{{ route('dashboard') }}" size="lg">Open dashboard</april:button-link>
                    @else
            ...
            <div class="mt-20 grid gap-4 md:grid-cols-3">
                @foreach ([['Native Laravel', 'Use routes, middleware, policies, migrations, and vendor publishing as you already do.'], ['Blade first', 'Build server-rendered pages with controllers, forms, validation, redirects, and familiar Laravel conventions.'], ['April UI included', 'Start with accessible, composable Blade components and customize their published views when needed.']] as [$title, $description])
                    <april:card>
